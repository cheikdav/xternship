...
class Type_Recherche(forms.Form):
    ...
    def clean_my_field(self):
        ...
        return self.cleaned_data['type_stage']
    
class Categorie_Recherche(forms.Form):
    cat_wanted = forms.ModelMultipleChoiceField(label = 'Categories', queryset = Categorie.objects.all(), widget=forms.CheckboxSelectMultiple(), initial = Categorie.objects.all()) 
    def clean_my_field(self):
        if len(self.cleaned_data['cat_wanted']) == 0:
            raise forms.ValidationError('Choisir au moins une categorie')
        return self.cleaned_data['cat_wanted']
